#
# pull_measurement_concept_attributes.R
#
# Connects to the FinnGen OMOP CDM (via FinnGenUtilsR, environment = "build") and pulls,
# for every STANDARD concept in the Measurement domain, one row per concept_id with:
...
# LOINC attributes, so they only add noise here. Narrow further with an extra
# `AND vocabulary_id = 'LOINC'` / `AND concept_class_id = 'Lab Test'` in the Step 1 / Step 2
# SQL below if only LOINC lab tests are wanted.
...
outputFolder <- "EXPERIMENTS/measurement_attributes/output"
dir.create(outputFolder, recursive = TRUE, showWarnings = FALSE)
pathToDiscoveredAttributesTSV <- file.path(outputFolder, "measurement_attribute_relationships.tsv")
pathToConceptAttributesTSV    <- file.path(outputFolder, "measurement_concept_attributes.tsv")
...
message("Discovered ", nrow(relationshipCounts), " relationship_id types attached to standard '", domainId, "' concepts.")
message("Full list written to: ", pathToDiscoveredAttributesTSV)
...
# "finding" concepts, not on lab test protocols. Step 2 keeps only LOINC's 6 core axes — the
# same ones Athena shows on a lab test's concept page (component, method, property, scale
# type, system, time aspect) — since those are what actually describe a lab measurement.
...
message("Using ", length(attributeRelationshipIds), " core LOINC attribute relationships: ", paste(attributeRelationshipIds, collapse = ", "))
...
message("Flagged ", length(panelConceptIds), " concepts as panels (have 'Panel contains' relationships).")

#
# --- Base measurement concepts (concept_id, concept_name, vocabulary_id) ----------------
#
sql_concepts <- "
SELECT
    concept_id,
...
message("Wrote ", nrow(measurementConceptAttributes), " standard '", domainId, "' concepts with attributes to: ", pathToConceptAttributesTSV)
