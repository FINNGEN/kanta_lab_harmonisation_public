#
# Reads measurement_concept_attributes.tsv (produced by pullMeasurementConceptAttributes.R)
# and writes a markdown summary with:
...
# the 6 core LOINC axes pulled by pullMeasurementConceptAttributes.R (excludes
...
# shown at the top of that column's chapter (see this step's README.md for the full write-up)
...
pathToSummaryMD <- file.path(outDir, "measurement_concept_attributes_summary.md")
...
data <- readr::read_tsv(inputFile, col_types = readr::cols(.default = readr::col_character()))
ParallelLogger::logInfo("Read ", nrow(data), " rows, ", ncol(data), " columns from ", inputFile)
...
md <- paste0(
  ...
  "Summary of `", inputFile, "`, generated by `summariseMeasurementConceptAttributes.R`.\n\n",
  "## Overall table stats\n\n",
  "- Rows (concepts): ", nRows, "\n",
  "- Columns: ", nCols, "\n\n",
...
ParallelLogger::logInfo("Wrote summary for ", nRows, " rows / ", nCols, " columns to ", pathToSummaryMD)
